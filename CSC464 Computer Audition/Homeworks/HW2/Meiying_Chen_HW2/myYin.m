function [pitch, ap_pwr, rms_val] = myYin(wavData, fs, tHop, tW, f0Min, f0Max, dp_th)
    ...
    for k = 1:nframes
        l = find(d_norm(k,:) < dp_th,1); % find the value below threshold
        % if no value below threshold, use global min
        if(isempty(l) == 1)
            [~,l] = min(d_norm(k,:));
            %l = inf;
        end
    ...
    disp('plotting')
    % plot calculating process at frame 1000
   
    figure(1);
    plot(0:length(x_frame(1000,:))-1, x_frame(1000,:));grid on;
    xlabel('Samples');
    ylabel('Amplitude');
    figure(2);
    plot(0:length(d(1000,:))-1, d(1000,:));grid on;
    xlabel('Lags');
    ylabel('Difference function');
    figure(3);
    plot(0:length(d_norm(1000,:))-1, d_norm(1000,:));grid on;
    xlabel('Lags');
    ...
    %{
     rms_cor = rms_val > 0.0210;
     ap_pwr_cor = ap_pwr < 0.9995;
     corr = rms_cor | ap_pwr_cor;
     pitch_cor = pitch .* corr;
     figure(6);
     plot(time, pitch_cor);grid on;
     xlabel('Time(s)');
     ylabel('Pitch(Hz)');
    %}
